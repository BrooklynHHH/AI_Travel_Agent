<template>
  ...
  try {
    ...
    const response = await fetch('https://mify-be.pt.xiaomi.com/api/v1/chat-messages', {
      method: 'POST',
      headers: {
        'Authorization': 'Bearer app-qqmztNVKBo3KCCODgppJuhwe',
        ...
        if (line.startsWith('data: ')) {
          ...
          if (eventData.event === 'node_finished' && eventData.data.title.includes('输出')) {
            ...
            if (lastAssistantIndex >= 0) {
              ...
              // 处理角色卡片
              if (outputs.output_role) {
                console.log('Received role card:', outputs.output_role);
            ...
            console.log('Received answer:', eventData.answer);
