...
export default {
  ...
  setup() {
    ...
    // 创建新的agent输出卡片（每次调用都创建新卡片）
    const createNewAgentOutput = (message, agentName) => {
      // 计算该agent的调用次数
      const callCount = message.agentOutputs.filter(output => output.agentName === agentName).length + 1
      ...
      const agentOutput = {
        ...
        endTime: null,
        status: 'processing',
        id: generateId(), // 添加唯一ID以区分不同的调用
        callIndex: callCount, // 添加调用次数
        sessionId: `${agentName}_${Date.now()}`, // 添加会话标识
    ...
    // 判断是否应该创建新卡片
    const shouldCreateNewCard = (message, agentName) => {
      const existingOutputs = message.agentOutputs.filter(output => output.agentName === agentName)
      ...
      const timeSinceLastUpdate = Date.now() - (lastOutput.lastUpdateTime || lastOutput.startTime)
      
      // 如果距离上次更新超过3秒，或者上一个卡片已完成，创建新卡片
      if (timeSinceLastUpdate > 3000 || lastOutput.status === 'completed') {
        return true
      }
    ...
    // 获取或创建agent输出卡片
    const getOrCreateAgentOutput = (message, agentName, forceNew = false) => {
      // 如果强制创建新卡片，直接创建
      if (forceNew) {
        return createNewAgentOutput(message, agentName)
      }
      ...
      for (let i = message.agentOutputs.length - 1; i >= 0; i--) {
        const output = message.agentOutputs[i]
        if (output.agentName === agentName && 
            output.status === 'processing' && 
            (now - output.startTime) < timeWindow) {
          return output
        }
      }
    ...
    const processStreamData = (data, currentMessage) => {
      ...
      switch (data.type) {
        ...
        case 'agent_start':
          ...
          // 立即创建agent卡片，让用户看到智能体开始工作
          if (data.agent && data.agent !== 'tools') {
            console.log(`🎯 [创建卡片] 为 ${data.agent} 创建新卡片`)
            const agentOutput = createNewAgentOutput(currentMessage, data.agent)
            // 设置初始状态为处理中，即使还没有内容
            agentOutput.status = 'processing'
            agentOutput.content = '' // 初始为空，等待内容填充
            scrollToBottom()
          }
          break
          ...
          if (data.data && data.data.chunk && Array.isArray(data.data.chunk) && data.data.chunk.length >= 2) {
            ...
            if (langgraph_node === "tools") {
              ...
              if (toolName) {
                ...
                loadingText.value = `正在调用工具：${toolName}...`
                
                // 检查是否需要创建新的工具卡片
                const isNewToolCall = shouldCreateNewCard(currentMessage, 'tools')
                let toolOutput = getOrCreateAgentOutput(currentMessage, 'tools', isNewToolCall)
                
                // 如果是新卡片，添加工具名称标题
                if (isNewToolCall || !toolOutput.content) {
                  toolOutput.content = `## 🔧 ${toolName}\n\n`
                }
                
                if (content) {
                  toolOutput.content += content + '\n\n'
                  toolOutput.lastUpdateTime = Date.now()
                }
              }
            }
            ...
            if (langgraph_node === "agent") {
              ...
              if (match) {
                ...
                // 检查是否需要创建新的agent卡片
                const isNewAgentCall = shouldCreateNewCard(currentMessage, agentName)
                let agentOutput = getOrCreateAgentOutput(currentMessage, agentName, isNewAgentCall)
                
                if (content) {
                  agentOutput.content += content
                  agentOutput.lastUpdateTime = Date.now()
                }
              }
